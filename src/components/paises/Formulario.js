...
class Formulario extends React.Component{
  ...
  componentDidUpdate(prevProps){
    if(this.props.paisSeleccionado !== prevProps.paisSeleccionado) {
      if(this.props.paisSeleccionado !== null ){
        this.setState({form: this.props.paisSeleccionado[0]})
      }
  ...
  insertar= ()=>{
    ...
    if(this.props.paisSeleccionado === null)
    {
      this.props.guardarDatosPaises(this.state.form)
    }
    /* Sino agregar */
    else {
      this.props.actualizarPais(this.state.form)
    }
    this.setState(({
      form: {
        id: Date.now(),
